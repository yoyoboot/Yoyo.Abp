...
if ($currentVersion.StartsWith('7.', [System.StringComparison]::OrdinalIgnoreCase)) {
    Assert-Equal -Expected '33-compat' -Actual $currentSelection['profile'] -Context 'current repo profile'
}
elseif ($currentVersion.StartsWith('9.', [System.StringComparison]::OrdinalIgnoreCase)) {
    Assert-Equal -Expected 'v9-stable' -Actual $currentSelection['profile'] -Context 'current repo profile'
}
elseif ($currentVersion.StartsWith('10.', [System.StringComparison]::OrdinalIgnoreCase)) {
    Assert-Equal -Expected 'v10-stable' -Actual $currentSelection['profile'] -Context 'current repo profile'
}
else {
    throw "Unexpected current version for root repo test: $currentVersion"
}
...
Assert-GenerationSelection -Version '7.4.0' -ExpectedGeneration 'v7-net6' -ExpectedProfile '33-compat' -ExpectedTargetFramework 'net6.0' -ExpectedStageKeepProjects @(
