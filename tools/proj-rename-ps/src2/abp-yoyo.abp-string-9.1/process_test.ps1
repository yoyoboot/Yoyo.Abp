...
function ReplaceTests {
    ...
    $content = $content -creplace [Regex]::Escape('RandomHelper.GetRandom()'), 'RandomHelper.GetRandom().ToString()'
    ...
    if ([regex]::IsMatch($path, [Regex]::Escape('Abp.ZeroCore.Tests')) -or [regex]::IsMatch($path, [Regex]::Escape('Abp.ZeroCore.SampleApp'))) {
        ...
        if ($path.EndsWith('SimpleEntityHistory_Test.cs')) {
            ...
            $content = $content -creplace [Regex]::Escape('<Employee>'), '<Employee,int>'
        }

        ## IOfficeAppService.cs
        if ($path.EndsWith('IOfficeAppService.cs')) {
            $content = $content -creplace [Regex]::Escape('<Office>'), '<Office, int>'
        }
        ## OfficeListDto.cs
        if ($path.EndsWith('OfficeListDto.cs')) {
            $content = $content -creplace [Regex]::Escape('EntityDto'), 'EntityDto<long>'
        }
        ## Employee.cs
        if ($path.EndsWith('Employee.cs')) {
            $content = $content -creplace [Regex]::Escape('FullAuditedEntity'), 'FullAuditedEntity<int>'
        }
        ## Office.cs
        if ($path.EndsWith('Office.cs')) {
            $content = $content -creplace [Regex]::Escape(': Entity,'), ': Entity<int>,'
        }
        ## OfficeTranslation.cs
        if ($path.EndsWith('OfficeTranslation.cs')) {
            $content = $content -creplace [Regex]::Escape(': Entity<string>,'), ': Entity<long>,'
        }
    }
    if (
...
"@
    ...
    $content = $content -creplace [Regex]::Escape('ShouldBe(1); // Not sure about that?'), 'ShouldBe("1"); // Not sure about that?'
