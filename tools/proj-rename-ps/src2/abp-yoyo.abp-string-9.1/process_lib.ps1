...
function ReplaceEntitys {
    ...
    $content = $content -creplace [Regex]::Escape('AbpSession.TenantId.ToString("D")'), 'AbpSession.TenantId'
    ...
    if ($path.EndsWith('AutoMapExtensions.cs')) {
        $content = $content -creplace [Regex]::Escape('where TTranslation : class, IEntity, IEntityTranslation<TMultiLingualEntity, int>'), 'where TTranslation : class, IEntity<int>, IEntityTranslation<TMultiLingualEntity, int>'
        $content = $content -creplace [Regex]::Escape('<TEntity, int>'), '<TEntity, string>'
    }
    ## TenantInfo.cs
