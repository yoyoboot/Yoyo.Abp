...
namespace Abp.Dapper_Extensions
{
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        {
            var isSimpleType = DapperExtensionsReflectionHelper.IsSimpleType(id.GetType());
            var keys = classMap.Properties.Where(p => p.KeyType != KeyType.NotAKey);
            IDictionary<string, Func<object>> paramValues = null;
            var predicates = new List<IPredicate>();
            if (!isSimpleType)
            {
                paramValues = DapperExtensionsReflectionHelper.GetObjectValues(id);
            }
        ...
        protected static IPredicate GetEntityPredicate(IClassMapper classMap, object entity)
        {
            ...
            foreach (var kvp in DapperExtensionsReflectionHelper.GetObjectValues(entity).Where(property => notIgnoredColumns.Any(c => c.Name == property.Key)))
            {
                var fieldPredicate = Activator.CreateInstance(predicateType) as IFieldPredicate;
                fieldPredicate.Not = false;
                fieldPredicate.Operator = Operator.Eq;
        ...
        protected virtual DynamicParameters AddParameter<T>(T entity, DynamicParameters parameters, IMemberMap prop, bool useColumnAlias = false)
        {
            ...
            var parameter = DapperExtensionsReflectionHelper.GetParameter(typeof(T), SqlGenerator, prop.Name, propValue);
    ...
    (prop.MemberType.IsGenericType &&
     ...
     prop.MemberType.GetGenericArguments()[0] == typeof(bool)))
            {
        ...
        private object InsertIdentity<IdentityType>(IDbConnection connection, IDbTransaction transaction,
            int? commandTimeout, IClassMapper classMap, string sql, DynamicParameters dynamicParameters)
        {
            IEnumerable<IdentityType> result;

            if (SqlGenerator.SupportsMultipleStatements())
            {
                sql += SqlGenerator.Configuration.Dialect.BatchSeperator + SqlGenerator.IdentitySql(classMap);
                result = connection.Query<IdentityType>(sql, dynamicParameters, transaction, false, commandTimeout, CommandType.Text);
            }
            else
            {
                connection.Execute(sql, dynamicParameters, transaction, commandTimeout, CommandType.Text);
            ...
            IMemberMap triggerIdentityColumn, IList<IMemberMap> sequenceIdentityColumn)
        {
            ...
            if (triggerIdentityColumn != null || identityColumn != null)
            {
                ...
                {
                    keyValue = InsertIdentityByKeyMemberType(keyColumn.MemberType,connection, transaction, commandTimeout, classMap, sql, dynamicParameters);
                }
        ...
        private object InsertIdentityByKeyMemberType(Type keyColumnMemberType, IDbConnection connection, IDbTransaction transaction, int? commandTimeout, IClassMapper classMap, string sql, DynamicParameters dynamicParameters)
        {
            if (keyColumnMemberType == typeof(int))
            {
                return InsertIdentity<int>(connection, transaction, commandTimeout, classMap, sql, dynamicParameters);
            }

            if (keyColumnMemberType == typeof(long))
            {
                return InsertIdentity<long>(connection, transaction, commandTimeout, classMap, sql, dynamicParameters);
            }

            if (keyColumnMemberType == typeof(Guid))
            {
                return InsertIdentity<Guid>(connection, transaction, commandTimeout, classMap, sql, dynamicParameters);
            }
