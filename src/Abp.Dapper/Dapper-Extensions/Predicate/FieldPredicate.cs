...
namespace Abp.Dapper_Extensions.Predicate
{
    ...
    public class FieldPredicate<T> : ComparePredicate, IFieldPredicate
    {
        ...
        {
            var p = DapperExtensionsReflectionHelper.GetParameter(typeof(T), sqlGenerator, PropertyName, value);
        ...
        private string GetParameterName(ISqlGenerator sqlGenerator, IDictionary<string, object> parameters, string parameterPropertyName, Type parentType)
        {
            ...
            var propParam = DapperExtensionsReflectionHelper.GetParameter(parentType ?? typeof(T), sqlGenerator, parameterPropertyName, Value);
