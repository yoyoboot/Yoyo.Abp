...
namespace Abp.Dapper_Extensions.Predicate
{
    ...
    public class BetweenPredicate<T> : BasePredicate, IBetweenPredicate
    {
        public override string GetSql(ISqlGenerator sqlGenerator, IDictionary<string, object> parameters, bool isDml = false)
        {
            ...
            var parameter1 = DapperExtensionsReflectionHelper.GetParameter(typeof(T), sqlGenerator, PropertyName, Value.Value1);
            var parameter2 = DapperExtensionsReflectionHelper.GetParameter(typeof(T), sqlGenerator, PropertyName, Value.Value2);
