...
namespace Abp.Dapper_Extensions.Mapper
{
    ...
    public class ReferenceMap<T> : IReferenceMap<T>
    {
        ...
        {
            var binaries = DapperExtensionsReflectionHelper.GetBinaryExpressionsFromUnary(((UnaryExpression)expression));
            foreach (BinaryExpression binary in binaries)
            {
                ...
                var comparator = DapperExtensionsReflectionHelper.GetRelacionalComparator(binary.NodeType, Name);
        ...
        {
            var resultExpression = DapperExtensionsReflectionHelper.GetProperty(expression, true);
