...
namespace Abp.Dapper_Extensions
{
    public static class Predicates
    {
        ...
        {
            var propertyInfo = DapperExtensionsReflectionHelper.GetProperty(expression) as PropertyInfo;
        ...
        public static IFieldPredicate Field<T>(string propertyName, Operator op, object value, bool not = false, bool useColumPrefix = true,
            DatabaseFunction databaseFunction = DatabaseFunction.None, string databaseFunctionParameters = "")
        {
            var properties = DapperExtensionsReflectionHelper.GetNestedProperties<T>(propertyName, '.', out string propertyInfoName);
        ...
        {
            var propertyInfo = DapperExtensionsReflectionHelper.GetProperty(expression) as PropertyInfo;
            var propertyInfo2 = DapperExtensionsReflectionHelper.GetProperty(expression2) as PropertyInfo;
        ...
        public static ISort Sort<T>(Expression<Func<T, object>> expression, bool ascending = true)
        {
            ...
            return new Sort
            {
                PropertyName = propertyInfo.Name,
                Ascending = ascending,
                Properties = DapperExtensionsReflectionHelper.GetNestedProperties<T>(propertyInfo.Name, '.', out var _)
            };
        }
        ...
        {
            var propertyInfos = DapperExtensionsReflectionHelper.GetNestedProperties<T>(propertyName, '.', out var propertyInfoName);
