...
public class SqlGeneratorImpl : ISqlGenerator
{
    ...
    {
        var assemblyBuilder = DapperExtensionsReflectionHelper.CreateAssemblyBuilder(mapper.EntityType.Assembly.GetName().Name);
        var moduleBuilder = DapperExtensionsReflectionHelper.CreateModuleBuilder(assemblyBuilder, "VirtualModules.dll");

        var tbEntity = DapperExtensionsReflectionHelper.CreateTypeBuilder(moduleBuilder, mapper.EntityType.Name, mapper.EntityType);
        var virtualEntity = DapperExtensionsReflectionHelper.CreateVirtualType(tbEntity, mapper.EntityType);

        var tbMapper = DapperExtensionsReflectionHelper.CreateTypeBuilder(moduleBuilder, mapper.EntityType.Name + "Map", mapper.GetType());
        DapperExtensionsReflectionHelper.CreateMapType(tbMapper, virtualEntity, mapper.GetType());
