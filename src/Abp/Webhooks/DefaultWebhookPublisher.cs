...
namespace Abp.Webhooks
{
    public class DefaultWebhookPublisher : ApplicationService, IWebhookPublisher
    {
        ...
        public virtual async Task PublishAsync(string webhookName, object data, string tenantId, bool sendExactSameData = false)
        {
            var subscriptions = await _webhookSubscriptionManager.GetAllSubscriptionsIfFeaturesGrantedAsync(tenantId, webhookName);
            await PublishAsync(webhookName, data, subscriptions, sendExactSameData);
        }

        public virtual async Task PublishAsync(string[] tenantIds, string webhookName, object data, bool sendExactSameData = false)
        {
        ...
        public virtual void Publish(string webhookName, object data, string tenantId, bool sendExactSameData = false)
        {
            var subscriptions = _webhookSubscriptionManager.GetAllSubscriptionsIfFeaturesGranted(tenantId, webhookName);
            Publish(webhookName, data, subscriptions, sendExactSameData);
        }

        public virtual void Publish(string[] tenantIds, string webhookName, object data, bool sendExactSameData = false)
        {
        ...
        protected virtual async Task<WebhookEvent> SaveAndGetWebhookAsync(string tenantId, string webhookName, object data)
        {
        ...
        protected virtual WebhookEvent SaveAndGetWebhook(string tenantId, string webhookName, object data)
        {
            var webhookInfo = new WebhookEvent
            {
                Id = _guidGenerator.Create(),
