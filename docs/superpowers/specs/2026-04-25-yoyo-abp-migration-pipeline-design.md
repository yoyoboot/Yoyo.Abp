...
7. 先重建并验证 `7.4` 生成路径，再推进 `v9.4.2 / .NET 8`；
8. 在 `.NET 8` 版本稳定后，再评估 `.NET 10` 路线。
...
官方 upstream 版本优先通过 tag、临时 worktree 或只读输入目录表示，不强制长期维护 `upstream/*`、`base/*`、`product/*` 多层分支。
...
目标：在 `.NET 8` 版本稳定后，再评估 `.NET 10` 路线。
...
应对：先完成 `.NET 8 / v9.4.2`，稳定后再评估 `.NET 10`。
...
4. 后续升级应以“官方 upstream 输入 → 迁移引擎生成 Yoyo 发行版 → 下游消费验证”为标准路径；
5. `Yoyo.Abp.*` 包名/程序集名和默认 `string` 主键语义是长期永久差异；
6. 分支策略应保持精简，不按理论层级过度拆分；
7. 路线优先级为：固化 `7.3.0.12` 生产基线 → 重建 `7.4` 标准生成路径 → 推进 `v9.4.2 / .NET 8` → 稳定后评估 `.NET 10`。
